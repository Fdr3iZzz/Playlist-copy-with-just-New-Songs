(async function() {
    ...
    function createDbCoreTable(tableSchema) {
      ...
      function mutate({ trans, type: type2, keys: keys2, values, range }) {
        return new Promise((resolve, reject) => {
          ...
          const { length: length2 } = keys2 || values || { length: 1 };
  ...
  var SettingsSection = class {
    constructor(name, settingsId, initialSettingsFields = {}) {
      ...
      this.Field = (props2) => {
        const id = `${this.settingsId}.${props2.nameId}`;
        ...
        return /* @__PURE__ */ import_react.default.createElement("div", {
          ...
          htmlFor: id
        }, props2.field.description || "")), /* @__PURE__ */ import_react.default.createElement("div", {
          className: "x-settings-secondColumn"
        }, props2.field.type === "input" ? /* @__PURE__ */ import_react.default.createElement("input", {
          ...
          onChange: (e) => {
            ...
            const onChange = props2.field.events?.onChange;
            if (onChange)
              onChange(e);
          }
        }) : props2.field.type === "button" ? /* @__PURE__ */ import_react.default.createElement("span", null, /* @__PURE__ */ import_react.default.createElement("button", {
          id,
          className: "Button-sm-buttonSecondary-isUsingKeyboard-useBrowserDefaultFocusStyle x-settings-button",
          ...props2.field.events,
          onClick: (e) => {
            setValue();
            const onClick = props2.field.events?.onClick;
            if (onClick)
              onClick(e);
          },
          type: "button"
        ...
        }))) : props2.field.type === "dropdown" ? /* @__PURE__ */ import_react.default.createElement("select", {
        ...
        }, props2.field.options.map((option, i) => {
  ...
  var Settings = class {
    constructor() {
      this.settings = new SettingsSection("Filter known tracks", "filter-known-tracks-settings");
    }
    initializeSettings() {
      this.settings.addToggle("welcome-user-toggle" /* welcomeUserToggle */, "Cute little welcome message", false);
      this.settings.addInput("welcome-user-name" /* welcomeUserName */, "Name for the welcome message", "Your Name");
      const defaultValue = "Original playlist from: {creatorName} | Playlist created via a Spicetify extension developed by Franz3";
      this.settings.addInput("playlist-description" /* playlistDescription */, "Description of the playlist created", defaultValue);
      this.settings.addInput("folder-name" /* folderName */, "Name of the folder where playlists will be saved in", "New Songs");
      this.settings.addToggle("self-owned-toggle" /* selfOwnedToggle */, "Filter to only count self-owned playlists rather then the whole library (needs a restart or database resynchronization)", true);
    ...
    isWelcomeUserToggled() {
      return this.settings.getFieldValue("welcome-user-toggle" /* welcomeUserToggle */);
    }
    getWelcomeUserName() {
      return this.settings.getFieldValue("welcome-user-name" /* welcomeUserName */);
    }
    getPlaylistDescription(artistName) {
      const description = this.settings.getFieldValue("playlist-description" /* playlistDescription */);
      return description.replace("{creatorName}", artistName);
    }
    getFolderName() {
      return this.settings.getFieldValue("folder-name" /* folderName */);
    }
    isSelfOwnedToggled() {
      return this.settings.getFieldValue("self-owned-toggle" /* selfOwnedToggle */);
    }
  };
  function applyClassesToButton() {
    ...
    const button = document.getElementById("filter-known-tracks-settings.resync");
    if (button) {
  ...
  async function getTrackObject(uris) {
    var _a;
    if (uris.length <= 0) {
    ...
    for (const response of bulkResponse) {
      if (response) {
        if (((_a = response == null ? void 0 : response.tracks) == null ? void 0 : _a.length) <= 0) {
          console.log("response tracks array is empty");
          continue;
        }
      }
      const trackObject = await createTrackObject(response == null ? void 0 : response.tracks);
      if (trackObject) {
        if ((trackObject == null ? void 0 : trackObject.length) >= 1)
          trackObjects.push(...trackObject);
      }
    }
    return trackObjects.flat();
  }
  async function createTrackObject(responses) {
    var _a;
    const trackObjects = [];
    for (const response of responses) {
      if (!(response == null ? void 0 : response.uri) || !((_a = response == null ? void 0 : response.external_ids) == null ? void 0 : _a.isrc) || !(response == null ? void 0 : response.name) || !(response == null ? void 0 : response.artists) || !(response == null ? void 0 : response.duration_ms)) {
        console.log("Error while getting TrackObject: ");
        console.log(response);
        if (response == null ? void 0 : response.uri) {
  ...
  function addTracksToPlaylist(playlistUri, trackUris) {
  ...
  async function getTracksFromContextMenu(playlistUris) {
  ...
  async function getTracksFromPlaylist(playlistUri) {
  ...
  async function isPlaylistSuitable(playlistItem, ignoreUserSettings = false) {
  ...
  async function getPlaylistInformation(playlistUri) {
    var _a, _b, _c, _d;
    const metadata = await Spicetify.Platform.PlaylistAPI.getMetadata(playlistUri);
    let playlistName = metadata == null ? void 0 : metadata.name;
    let creatorName = (_a = metadata == null ? void 0 : metadata.owner) == null ? void 0 : _a.displayName;
    let playlistImageId;
    playlistImageId = ((_b = metadata == null ? void 0 : metadata.images[0]) == null ? void 0 : _b.url).split(":").slice(2).join(":");
    if (!playlistImageId) {
      playlistImageId = ((_c = metadata == null ? void 0 : metadata.images[0]) == null ? void 0 : _c.url).split("/").slice(4).join("/");
    }
    if (!playlistName) {
    ...
    if (((_d = metadata == null ? void 0 : metadata.images[0]) == null ? void 0 : _d.url).split(":")[1] === "mosaic") {
      playlistImageId = void 0;
    }
    if (playlistName === "") {
    ...
    return {
      "playlistName": playlistName != null ? playlistName : "Error getting name",
      "creatorName": creatorName != null ? creatorName : "Error getting name",
      "playlistImage": playlistImageId
    };
  }
  async function compareIsrc(trackObject) {
    const localUri = trackObject == null ? void 0 : trackObject.uri;
    const localIsrc = trackObject == null ? void 0 : trackObject.isrc;
    if (!localUri || !localIsrc)
      return false;
    const databaseTrackObject = await db.webTracks.get(localUri);
    if (!(databaseTrackObject == null ? void 0 : databaseTrackObject.isrc))
      return false;
    return databaseTrackObject.isrc === localIsrc;
  }
  var contextMenu = new Spicetify.ContextMenu.Item(
  ...
  async function onPlaylistContextMenu(playlistUris) {
    const typedPlaylistUri = playlistUris;
    ...
    for (const playlistUri of typedPlaylistUri) {
      ...
      let trackUrisToAdd;
      const playlistItem = await Spicetify.Platform.PlaylistAPI.getPlaylist(playlistUri).then((response) => response.metadata);
      ...
      if (!playlistImage) {
        await Spicetify.Platform.PlaylistAPI.setAttributes(
          createdPlaylistUri,
      ...
      const folderUri = await getFolder(folderName);
      if (!folderUri)
        return;
  ...
  async function movePlaylist(playlistUri, folderUri) {
  ...
  async function getFolder(folderName) {
    ...
    const items = content == null ? void 0 : content.items;
    ...
    if (!(createdFolder == null ? void 0 : createdFolder.uri)) {
  ...
  async function compareIsrcToDatabase(trackUri) {
  ...
  function progressFeedback(length2, i) {
    if (Math.floor(length2 / 4) === i)
      Spicetify.showNotification("1/4 of tracks processed");
    if (Math.floor(length2 / 2) === i)
      Spicetify.showNotification("1/2 of tracks processed");
    if (Math.floor(length2 / 4) * 3 === i)
      Spicetify.showNotification("3/4 of tracks processed");
  }

  // src/getInitialTracks.ts
  async function getAllLocalTracks() {
    ...
    for (const item of userContents.items) {
      const uris = await processItem(item);
      if (uris) {
        localUris.push(...uris);
      }
    }
    await getLikedTracks().then((uris) => {
      if (uris) {
        localUris.push(...uris);
      }
    });
  ...
  async function processItem(item) {
    ...
    while (stack.length > 0) {
      ...
      if (!currentItem)
        continue;
      if (currentItem.type === "playlist" /* Playlist */) {
        const userPlaylist = await isPlaylistSuitable(currentItem);
        if (userPlaylist) {
          const tracks = await getTracksFromPlaylist(currentItem.uri);
          trackUris.push(...tracks);
        }
      } else if (currentItem.type === "folder" /* Folder */) {
  ...
  // src/database.ts
  var db = new class extends Dexie$1 {
    constructor() {
      super("library-data");
  ...
  async function resyncDatabaseAndMap() {
    Spicetify.showNotification("ReSync started");
    const urisToSync = await getAllLocalTracks();
    counter.clear();
    ...
    for (const localUri of counter.keys()) {
      if (allDatabaseUris.includes(localUri))
        continue;
  ...
  var counter = /* @__PURE__ */ new Map();
  function initializeCounter(urisToSync) {
    if (urisToSync.length <= 0)
      return;
  ...
  async function trackEventHandler(event) {
    var _a, _b, _c;
    if (!((_a = event == null ? void 0 : event.data) == null ? void 0 : _a.operation))
      return console.log("Unable to tell event type");
    if (event.data.operation === "add") {
      if (!((_b = event.data) == null ? void 0 : _b.uris) || event.data.uris.length <= 0)
        return console.log("Unable to get relevant tracks");
      const uris = event.data.uris;
      await addTracksToDatabase(uris);
    } else if (event.data.operation === "remove") {
      if (!((_c = event.data) == null ? void 0 : _c.items) || event.data.items.length <= 0)
        return;
      const urisToDelete = [];
  ...
  async function playlistEventHandler(event) {
    ...
    if (event.data.operation !== "remove")
      return;
    if (!((_b = event.data) == null ? void 0 : _b.items) || event.data.items.length <= 0) {
      console.log("Unable to get relevant playlist");
      return;
    }
    const urisToDelete = [];
    for (const playlist of event.data.items) {
      await getTracksFromPlaylist(playlist.uri).then((uris) => urisToDelete.push(...uris));
    }
    if (urisToDelete.length > 0) {
  ...
  async function likedEventHandler(event) {
    ...
    if (event.data.operation === "add") {
      ...
      if (!((_c = event.data) == null ? void 0 : _c.uris) || event.data.uris.length <= 0)
        return console.log("Unable to get relevant tracks");
      const urisToDelete = [];
  ...
  async function addTracksToDatabase(uris) {
    if (uris.length <= 0)
      return;
    const urisToAdd = [];
  ...
  function removeTracksFromDatabase(uris) {
    if (uris.length <= 0)
      return;
    const urisToRemove = [];
    for (let i = 0; i <= uris.length - 1; i++) {
  ...
  async function main() {
    ...
    while (!((_b = (_a = Spicetify == null ? void 0 : Spicetify.Platform) == null ? void 0 : _a.PlaylistAPI) == null ? void 0 : _b.getPlaylist)) {
